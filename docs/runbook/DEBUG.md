...
5. 홈 상단 광고는 로컬에서 티스토리 수익 치환자가 실제로 치환되지 않을 수 있으므로, `#pageHome > .home-revenue-section`이 별도 박스 스타일 없이 자연스럽게 배치되는지 확인합니다.
6. 구글 광고 관련 별도 JS는 넣지 않으므로, 광고 확인은 티스토리 수익 치환자(`[##_revenue_list_upper_##]`, `[##_revenue_list_lower_##]`)가 실제 환경에서 정상 치환되는지 기준으로 점검합니다.
...
- 홈 상단 광고 위치 확인:
  - `#pageHome > .home-revenue-section`이 홈 첫 섹션으로 배치되는지 확인
  - `[##_revenue_list_upper_##]` 주변에 별도 배경/그림자/라운드 박스가 생기지 않는지 확인
- 본문 광고 개수 확인:
  - 비홈 페이지에서는 `#ttHomeLanding`이 `display:none`으로 완전히 숨겨져 홈 상단 광고가 함께 렌더링되지 않는지 확인
  - `#pageArticle`에서 `#ttMidAd`가 숨겨지고 본문 구글 광고가 1개만 보이는지 확인
- 링크 동작:
...
- 증상: 홈 상단 광고가 헤더와 겹치거나 너무 위에 붙음
  - 원인: `#pageHome` 상단 패딩 또는 인접 섹션 마진 규칙이 다른 CSS에 덮임
  - 조치: `src/style/style.css`의 `#pageHome`, `#pageHome > * + *`, `.home-revenue-section` 규칙을 확인
- 증상: 본문 페이지에서 구글 광고가 2개 이상 보임
  - 원인: 본문 하단 수동 슬롯 `#ttMidAd`가 계속 노출되거나, 티스토리/외부 광고 iframe이 상세 본문 내부에 추가 삽입됨
  - 조치: `src/function/content-effects.js`에서 `toggleMidAdSlot("article")`가 실행되어 `#ttMidAd`를 숨기는지 확인하고, 외부 삽입 광고는 티스토리 관리자 설정을 함께 점검
- 증상: 자동 광고를 켰는데도 특정 위치(`backdrop`)에 구글 광고가 고정으로 안 뜸
  - 원인: 자동 광고는 코드가 아니라 애드센스가 전체 레이아웃을 보고 위치를 결정하므로 특정 섹션 노출을 보장하지 않음
  - 조치: 특정 위치 고정이 필요하면 자동 광고 대신 해당 위치용 수동 광고 단위를 별도로 사용해야 함
